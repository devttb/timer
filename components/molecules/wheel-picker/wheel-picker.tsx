...
// import type { WheelPickerOption, WheelPickerProps, WheelPickerWrapperProps } from './types';
import { useControllableState } from './use-controllable-state';
import classes from './wheel-picker.module.css';
...
const WheelPickerWrapper: React.FC<WheelPickerWrapperProps> = ({ className, children }) => {
  return (
    <div className={classes.wrapper} data-rwp-wrapper>
      {children}
    </div>
  );
};
...
const WheelPicker: React.FC<WheelPickerProps> = ({
  ...
  visibleCount: countProp = 12,
  dragSensitivity: dragSensitivityProp = 3,
  scrollSensitivity: scrollSensitivityProp = 1,
  optionItemHeight: optionHeightProp = 60,
  classNames,
}) => {
  ...
  const renderWheelItems = useMemo(() => {
    ...
    if (infiniteProp) {
      for (let i = 0; i < quarterCount; ++i) {
        ...
        items.unshift(
          renderItem(options[options.length - i - 1], prependIndex, itemAngle * (i + 1))
        );
  ...
  const handleDragStartEvent = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      const isTargetValid =
        !!containerRef.current?.contains(e.target as Node) || e.target === containerRef.current;
  ...
  return (
    <div ref={containerRef} className={classes.container} style={{ height: containerHeight }}>
      <ul ref={wheelItemsRef} className={classes.options} data-rwp-options>
        {renderWheelItems}
      </ul>
      ...
      >
        <ul
          className={classes.highlightList}
